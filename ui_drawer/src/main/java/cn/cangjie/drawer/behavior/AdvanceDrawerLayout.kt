package cn.cangjie.drawer.behavior
...
open class AdvanceDrawerLayout : DrawerLayout {
    var settings = hashMapOf<Int, Setting?>()
    ...
    private fun init(context: Context, attrs: AttributeSet?, defStyle: Int) {
        ...
        if (!isInEditMode) if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
    ...
    private fun updateSlideOffset(drawerView: View, slideOffset: Float) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
            isRtl =
                layoutDirection == View.LAYOUT_DIRECTION_RTL || window.decorView.layoutDirection == View.LAYOUT_DIRECTION_RTL || resources.configuration.layoutDirection == View.LAYOUT_DIRECTION_RTL
        }
