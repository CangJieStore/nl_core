...
object LunarCalendar {
     ...
     * 公历每月前的天数
     */
    private val DAYS_BEFORE_MONTH = intArrayOf(0, 31, 59, 90, 120, 151, 181,
        ...
        while (i < 13) {
            ...
            if (iPos >= dayOffset) {
                ...
                if (dayOffset > iPos) solarInfo[2] = dayOffset - iPos else if (dayOffset == iPos) {
                    if (year % 4 == 0 && i == 2) solarInfo[2] = DAYS_BEFORE_MONTH[i] - DAYS_BEFORE_MONTH[i - 1] + 1 else solarInfo[2] = DAYS_BEFORE_MONTH[i] - DAYS_BEFORE_MONTH[i - 1]
