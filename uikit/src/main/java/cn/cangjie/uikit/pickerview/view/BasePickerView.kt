...
import cn.cangjie.uikit.pickerview.listener.OnDismissListener
import cn.cangjie.uikit.pickerview.utils.PickerViewAnimateUtil.getAnimationResource
...
open class BasePickerView(private val context: Context) {
    ...
    protected fun initViews() {
        ...
        if (isDialog()) {
            ...
            //这个是真正要加载选择器的父布局
            dialogContainerLayout = dialogView!!.findViewById<View>(R.id.content_container) as ViewGroup
            //设置对话框 默认左右间距屏幕30
            ...
            //将控件添加到decorView中
            rootView = layoutInflater.inflate(R.layout.layout_basepickerview, mPickerOptions!!.decorView, false) as ViewGroup
            ...
            //这个是真正要加载时间选取器的父布局
            dialogContainerLayout = rootView!!.findViewById<View>(R.id.content_container) as ViewGroup
            dialogContainerLayout!!.layoutParams = params
        }
    ...
    fun setKeyBackCancelable(isCancelable: Boolean) {
        val v: ViewGroup? = if (isDialog()) {
            dialogView
        } else {
            rootView
        }
        v!!.isFocusable = isCancelable
        v.isFocusableInTouchMode = isCancelable
        if (isCancelable) {
            v.setOnKeyListener(onKeyBackListener)
        } else {
