...
class OptionsPickerView<T>(pickerOptions: PickerOptions) : BasePickerView(pickerOptions.context!!), View.OnClickListener {
    ...
    private fun initView(context: Context?) {
        ...
        if (mPickerOptions!!.customListener == null) {
            LayoutInflater.from(context).inflate(mPickerOptions!!.layoutRes, dialogContainerLayout)
            ...
            mPickerOptions!!.customListener!!.customLayout(LayoutInflater.from(context).inflate(mPickerOptions!!.layoutRes, dialogContainerLayout))
        }
        ...
        wheelOptions?.let {
            if (mPickerOptions!!.optionsSelectChangeListener != null) {
                it.setOptionsSelectChangeListener(mPickerOptions!!.optionsSelectChangeListener)
            }
            it.setTextContentSize(mPickerOptions!!.textSizeContent)
            it.setItemsVisible(mPickerOptions!!.itemsVisibleCount)
            it.setAlphaGradient(mPickerOptions!!.isAlphaGradient)
            it.setLabels(mPickerOptions!!.label1, mPickerOptions!!.label2, mPickerOptions!!.label3)
            it.setTextXOffset(mPickerOptions!!.x_offset_one, mPickerOptions!!.x_offset_two, mPickerOptions!!.x_offset_three)
            it.setCyclic(mPickerOptions!!.cyclic1, mPickerOptions!!.cyclic2, mPickerOptions!!.cyclic3)
            it.setTypeface(mPickerOptions!!.font)
    ...
    fun setPicker(options1Items: List<T>?,
        ...
        wheelOptions!!.setPicker(options1Items, options2Items as List<List<T>>?, options3Items as List<List<List<T>>>?)
        reSetCurrentItems()
    }
