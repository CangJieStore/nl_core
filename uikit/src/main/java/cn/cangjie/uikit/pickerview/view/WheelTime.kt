...
import cn.cangjie.uikit.pickerview.utils.ChinaDate.getLunarDays
import cn.cangjie.uikit.pickerview.utils.ChinaDate.getMonths
import cn.cangjie.uikit.pickerview.utils.ChinaDate.getYears
import cn.cangjie.uikit.pickerview.utils.ChinaDate.leapDays
import cn.cangjie.uikit.pickerview.utils.ChinaDate.leapMonth
import cn.cangjie.uikit.pickerview.utils.ChinaDate.monthDays
import cn.cangjie.uikit.pickerview.utils.LunarCalendar.lunarToSolar
import cn.cangjie.uikit.pickerview.utils.LunarCalendar.solarToLunar
...
class WheelTime(val view: View, private val type: BooleanArray, private val gravity: Int, private val textSize: Int) {
    ...
    private fun setLunar(year: Int, month: Int, day: Int, isLeap: Boolean, h: Int, m: Int, s: Int) {
        ...
        wv_year!!.adapter = ArrayWheelAdapter<Any?>(getYears(startYear, endYear) as List<Any?>?) // 设置"年"的显示数据
        ...
        wv_month!!.adapter = ArrayWheelAdapter<Any?>(getMonths(year) as List<Any?>?)
        wv_month!!.setLabel("")
        ...
        if (leapMonth(year) == 0) {
            wv_day!!.adapter = ArrayWheelAdapter<Any?>(getLunarDays(monthDays(year, month)) as List<Any?>?)
        } else {
            wv_day!!.adapter = ArrayWheelAdapter<Any?>(getLunarDays(leapDays(year)) as List<Any?>?)
        }
        wv_day!!.setLabel("")
        ...
        wv_minutes!!.adapter = NumericWheelAdapter(0, 59)
        ...
        wv_year!!.setOnItemSelectedListener(object : OnItemSelectedListener {
            override fun onItemSelected(index: Int) {
                ...
                // 判断是不是闰年,来确定月和日的选择
                wv_month!!.adapter = ArrayWheelAdapter<Any?>(getMonths(year_num) as List<Any?>?)
                if (leapMonth(year_num) != 0 && wv_month!!.currentItem > leapMonth(year_num) - 1) {
                ...
                var maxItem = 29
                if (leapMonth(year_num) != 0 && wv_month!!.currentItem > leapMonth(year_num) - 1) {
                    if (wv_month!!.currentItem == leapMonth(year_num) + 1) {
                        wv_day!!.adapter = ArrayWheelAdapter<Any?>(getLunarDays(leapDays(year_num)) as List<Any?>?)
                        maxItem = leapDays(year_num)
                    } else {
                        wv_day!!.adapter = ArrayWheelAdapter<Any?>(getLunarDays(monthDays(year_num, wv_month!!.currentItem)) as List<Any?>?)
                        maxItem = monthDays(year_num, wv_month!!.currentItem)
                    }
                } else {
                    wv_day!!.adapter = ArrayWheelAdapter<Any?>(getLunarDays(monthDays(year_num, wv_month!!.currentItem + 1)) as List<Any?>?)
                    maxItem = monthDays(year_num, wv_month!!.currentItem + 1)
                }
        ...
        wv_month!!.setOnItemSelectedListener(object : OnItemSelectedListener {
            override fun onItemSelected(index: Int) {
                ...
                val currentIndex = wv_day!!.currentItem
                var maxItem = 29
                if (leapMonth(year_num) != 0 && index > leapMonth(year_num) - 1) {
                    if (wv_month!!.currentItem == leapMonth(year_num) + 1) {
                        wv_day!!.adapter = ArrayWheelAdapter<Any?>(getLunarDays(leapDays(year_num)) as List<Any?>?)
                        maxItem = leapDays(year_num)
                    } else {
                        wv_day!!.adapter = ArrayWheelAdapter<Any?>(getLunarDays(monthDays(year_num, index)) as List<Any?>?)
                        maxItem = monthDays(year_num, index)
                    }
                } else {
                    wv_day!!.adapter = ArrayWheelAdapter<Any?>(getLunarDays(monthDays(year_num, index + 1)) as List<Any?>?)
                    maxItem = monthDays(year_num, index + 1)
                }
                if (currentIndex > maxItem - 1) {
                    wv_day!!.currentItem = maxItem - 1
                }
            ...
            override fun onItemSelected(index: Int) {
                ...
                if (startYear == endYear) {
                    ...
                    if (month_num == endMonth) {
                        //重新设置日
                        setReDay(currentYear, wv_month!!.currentItem + 1, 1, endDay, list_big, list_little)
                    } else {
    ...
    private fun setReDay(year_num: Int, monthNum: Int, startD: Int, endD: Int, list_big: List<String>, list_little: List<String>) {
        var endD = endD
    ...
    private val lunarTime: String
        private get() {
            ...
            if (leapMonth(year) == 0) {
                ...
                if (wv_month!!.currentItem + 1 - leapMonth(year) <= 0) {
                    month = wv_month!!.currentItem + 1
                } else if (wv_month!!.currentItem + 1 - leapMonth(year) == 1) {
