...
class CaptureHandler internal constructor(activity: Activity?, private val viewfinderView: ViewfinderView?, private val onCaptureListener: OnCaptureListener,
    ...
    private fun transform(originPoint: ResultPoint): ResultPoint {
        ...
        if (screenPoint!!.x < screenPoint.y) {
            ...
            x = originPoint.x * scaleX - max(screenPoint.x, cameraPoint.y) / 2
            y = originPoint.y * scaleY - min(screenPoint.y, cameraPoint.x) / 2
        } else {
            scaleX = 1.0f * screenPoint.x / cameraPoint!!.x
            scaleY = 1.0f * screenPoint.y / cameraPoint.y
            x = originPoint.x * scaleX - min(screenPoint.y, cameraPoint.y) / 2
            y = originPoint.y * scaleY - max(screenPoint.x, cameraPoint.x) / 2
        }
    ...
    init {
        decodeThread = DecodeThread(activity!!, cameraManager, this, decodeFormats as MutableCollection<BarcodeFormat?>?, baseHints, characterSet, this)
        decodeThread.start()
        state = State.SUCCESS
