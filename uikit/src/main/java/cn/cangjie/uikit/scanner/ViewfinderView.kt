...
import java.util.*
class ViewfinderView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : View(context, attrs, defStyleAttr) {
    /**
     * 画笔
     */
     ...
     * 扫描区域提示文本
     */
    private var labelText = "放入框内，自动扫描"
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        //初始化自定义属性信息
        val array = context.obtainStyledAttributes(attrs, R.styleable.ScannerView)
        maskColor = array.getColor(R.styleable.ScannerView_maskColor, ContextCompat.getColor(context, R.color.viewfinder_mask))
        frameColor = array.getColor(R.styleable.ScannerView_frameColor, ContextCompat.getColor(context, R.color.viewfinder_frame))
        cornerColor = array.getColor(R.styleable.ScannerView_cornerColor, ContextCompat.getColor(context, R.color.viewfinder_corner))
        laserColor = array.getColor(R.styleable.ScannerView_laserColor, ContextCompat.getColor(context, R.color.viewfinder_laser))
        resultPointColor = array.getColor(R.styleable.ScannerView_resultPointColor, ContextCompat.getColor(context, R.color.viewfinder_result_point_color))
        labelText = array.getString(R.styleable.ScannerView_labelText)!!
        labelTextColor = array.getColor(R.styleable.ScannerView_labelTextColor, ContextCompat.getColor(context, R.color.viewfinder_text_color))
        labelTextSize = array.getDimension(R.styleable.ScannerView_labelTextSize, TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 14f, resources.displayMetrics))
        labelTextPadding = array.getDimension(R.styleable.ScannerView_labelTextPadding, TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 24f, resources.displayMetrics))
        labelTextLocation = TextLocation.getFromInt(array.getInt(R.styleable.ScannerView_labelTextLocation, 0))
        isShowResultPoint = array.getBoolean(R.styleable.ScannerView_showResultPoint, false)
        frameWidth = array.getDimensionPixelSize(R.styleable.ScannerView_frameWidth, 0)
        frameHeight = array.getDimensionPixelSize(R.styleable.ScannerView_frameHeight, 0)
        laserStyle = LaserStyle.getFromInt(array.getInt(R.styleable.ScannerView_laserStyle, LaserStyle.LINE.mValue))
        gridColumn = array.getInt(R.styleable.ScannerView_gridColumn, 20)
        gridHeight = array.getDimension(R.styleable.ScannerView_gridHeight, TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 40f, resources.displayMetrics)).toInt()
        cornerRectWidth = array.getDimension(R.styleable.ScannerView_cornerRectWidth, TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 3f, resources.displayMetrics)).toInt()
        cornerRectHeight = array.getDimension(R.styleable.ScannerView_cornerRectHeight, TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 16f, resources.displayMetrics)).toInt()
        scannerLineMoveDistance = array.getDimension(R.styleable.ScannerView_scannerLineMoveDistance, TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 2f, resources.displayMetrics)).toInt()
        scannerLineHeight = array.getDimension(R.styleable.ScannerView_scannerLineHeight, TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 5f, resources.displayMetrics)).toInt()
        frameLineWidth = array.getDimension(R.styleable.ScannerView_frameLineWidth, TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 1f, resources.displayMetrics)).toInt()
        scannerAnimationDelay = array.getInteger(R.styleable.ScannerView_scannerAnimationDelay, 15)
        frameRatio = array.getFloat(R.styleable.ScannerView_frameRatio, 0.625f)
        array.recycle()
    ...
    private fun drawCorner(canvas: Canvas, frame: Rect) {
        ...
        //左上
        canvas.drawRect(frame.left - 2 * cornerRectWidth.toFloat(), frame.top - 2 * cornerRectWidth.toFloat(), frame.left - cornerRectWidth.toFloat(), frame.top + cornerRectHeight / 2.toFloat(), paint!!)
        canvas.drawRect(frame.left - cornerRectWidth.toFloat(), frame.top - 2 * cornerRectWidth.toFloat(), frame.left + cornerRectHeight / 2.toFloat(), frame.top - cornerRectWidth.toFloat(), paint!!)
        //右上
        canvas.drawRect(frame.right + 2 * cornerRectWidth.toFloat(), frame.top - 2 * cornerRectWidth.toFloat(), frame.right + cornerRectWidth.toFloat(), frame.top + cornerRectHeight / 2.toFloat(), paint!!)
        canvas.drawRect(frame.right - cornerRectHeight / 2.toFloat(), frame.top - 2 * cornerRectWidth.toFloat(), frame.right + cornerRectWidth.toFloat(), frame.top - cornerRectWidth.toFloat(), paint!!)
        //左下
        canvas.drawRect(frame.left - 2 * cornerRectWidth.toFloat(), frame.bottom + 2 * cornerRectWidth.toFloat(), frame.left + cornerRectHeight / 2.toFloat(), frame.bottom + cornerRectWidth.toFloat(), paint!!)
        canvas.drawRect(frame.left - 2 * cornerRectWidth.toFloat(), frame.bottom - cornerRectHeight / 2.toFloat(), frame.left - cornerRectWidth.toFloat(), frame.bottom + cornerRectWidth.toFloat(), paint!!)
        //右下
        canvas.drawRect(frame.right + 2 * cornerRectWidth.toFloat(), frame.bottom - cornerRectHeight / 2.toFloat(), frame.right + cornerRectWidth.toFloat(), frame.bottom + cornerRectWidth.toFloat(), paint!!)
        canvas.drawRect(frame.right - cornerRectHeight / 2.toFloat(), frame.bottom + cornerRectWidth.toFloat(), frame.right + 2 * cornerRectWidth.toFloat(), frame.bottom + 2 * cornerRectWidth.toFloat(), paint!!)
    }
    ...
    private fun drawLineScanner(canvas: Canvas, frame: Rect) {
        ...
        val linearGradient = LinearGradient(
                ...
                frame.left.toFloat(), scannerStart.toFloat() + scannerLineHeight,
    ...
    private fun drawGridScanner(canvas: Canvas, frame: Rect) {
        ...
        val linearGradient = LinearGradient((frame.left + frame.width() / 2).toFloat(), startY.toFloat(), (frame.left + frame.width() / 2).toFloat(), scannerStart.toFloat(), intArrayOf(shadeColor(laserColor), laserColor), floatArrayOf(0f, 1f), Shader.TileMode.CLAMP)
        //给画笔设置着色器
        paint!!.shader = linearGradient
