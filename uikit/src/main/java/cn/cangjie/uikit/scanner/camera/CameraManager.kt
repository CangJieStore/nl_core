...
import cn.cangjie.uikit.scanner.camera.open.OpenCameraInterface.open
...
class CameraManager(context: Context) {
    ...
    fun getFramingRect(): Rect? {
        if (framingRect == null) {
            ...
            framingRect = if (isFullScreenScan) {
                ...
                val size = (min(width, height) * framingRectRatio).toInt()
    ...
    interface OnSensorListener {
         ...
         */
        fun onSensorChanged(torch: Boolean, tooDark: Boolean, ambientLightLux: Float)
    }
